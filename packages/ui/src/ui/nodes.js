...
class RecorderSink {
  ...
  record(recording, bufferIndex, coordnodes) {
  ...
  setLabel(bufferIndex, label) {
    console.log('setLabel', bufferIndex, label);
...
/**
 * Recorder UI component with nodes interface. Multibuffer. Records to a global buffer.
 *
...
export default function recorder(options, source) {
  ...
  return {
    ...
    run(sink, scheduler) {
      ...
      app.$on('record', (recording, bufferIndex, coordnodes) => {
        recorderSink.record(recording, bufferIndex, coordnodes);
      });
