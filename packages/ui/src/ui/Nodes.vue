...
export default {
  ...
  methods: {
    ...
    record() {
      ...
      this.$emit('record', this.recording, this.nodeIndex, [this.nodes[this.nodeIndex].x, this.nodes[this.nodeIndex].y]);
    },
    addnode(x = 0.5, y = 0.5) {
    ...
    movenode(nodeIndex, x, y) {
      console.log('movenode', nodeIndex, x, y);
    ...
    setLabel(e) {
      this.$emit('label', this.nodeIndex, e.target.value);
    },
    select(circleId) {
      if (this.isDblClick()) {
