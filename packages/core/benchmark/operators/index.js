...
fs.readdirSync('./benchmark/operators')
...
suite
  ...
  .on('cycle', (event) => {
    ...
    result += ' | ';
    result += padr(14, ` ${(t.hz * iterations).toFixed(2)} op/s`);
    result += ' \xb1';
  ...
  .on('complete', () => {
    fs.writeFileSync('./benchmark/operators/README.md', log);
  })
