...
// Randomly select a segment on mousedown
c.index = mousedown(doc).rand().scale({ outmax: 8 }).map(x => Math.floor(x));

// The period annd duration of the segment playback can be set either relatively to the duration
// of the grain, or using an absolute value
c.periodAbs = 0.1;
c.periodRel = 0;
